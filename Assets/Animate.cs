...
public class Animate : MonoBehaviour
{
	...
	int s_frame;
	int e_frame;
	public List<ReadData.CSVReader> csvreader;
	...
	void GetInitInfo()
	{
		...
		Debug.Log(" HumanBodyBones.Hips " + HumanBodyBones.Hips);
	...
	void Start()
	{
		...
		if (System.IO.File.Exists (filename) == false) {
			Debug.Log("<color=blue>Error! Pos file not found(" + filename + "). Check Pos_filename in Inspector.</color>");
		}
		csvreader = ReadData.Read(filename);
		pos = csvreader[0].keypoints3d;
		Debug.Log(pos[0]);
		GetInitInfo();
		if (pos != null) {
